...
class Settings(BaseSettings):
    ...
    DATABASE_URL: str = os.getenv(
        "DATABASE_URL",
        "postgresql+asyncpg://aivo_admin:aivo_password@localhost:5432/"
        "aivo_production"
    )
    ...
    USE_LOCAL_MODELS: bool = (
        os.getenv("USE_LOCAL_MODELS", "false").lower() == "true"
    )
    ...
    AIVO_BRAIN_URL: str = os.getenv(
        "AIVO_BRAIN_URL", "http://aivo-brain:8014"
    )

    # Model Configuration
    CLONING_MODEL: str = os.getenv("CLONING_MODEL", "gpt-4o")
    PERSONALIZATION_MODEL: str = os.getenv(
