...
export const ChildCard: React.FC<ChildCardProps> = ({ child }) => {
  ...
  const age = useMemo(() => calculateAge(child.birthDate), [child.birthDate]);
  const learnerAppBaseUrl = import.meta.env.VITE_LEARNER_APP_URL || 'http://localhost:5176';
  const startLearningUrl = useMemo(() => {
    const params = new URLSearchParams({
      childId: child.id,
      childName: `${child.firstName} ${child.lastName}`,
      age: age.toString(),
      grade: child.grade === 0 ? 'Kindergarten' : `Grade ${child.grade}`,
    });

  const trimmedBase = learnerAppBaseUrl.replace(/\/+$/, '');
  const separator = trimmedBase.includes('?') ? '&' : '/?';
    return `${trimmedBase}${separator}${params.toString()}`;
  }, [age, child.firstName, child.lastName, child.grade, child.id, learnerAppBaseUrl]);
      ...
      <div className="block">
        ...
        <div className="mt-4 pt-4 border-t border-gray-100 flex gap-3">
          <button
            ...
            onClick={() => window.open(startLearningUrl, '_blank', 'noopener,noreferrer')}
          >
